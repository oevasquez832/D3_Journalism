...
d3.csv("data/correlations.csv", function(err, datacorr) {
    if(err) throw err;

    datacorr.forEach(function(data) {
...
xMin = d3.min(datacorr, function(data) {
    return +data.income * 0.95;
});

xMax = d3.max(datacorr, function(data) {
    return +data.income * 1.05;
});

yMin = d3.min(datacorr, function(data) {
    return +data.obesity * 0.98;
});

yMax = d3.max(datacorr, function(data) {
...
xLinearScale.domain([xMin, xMax]);
    ...
    var toolTip = d3
        ...
        .html(function(data) {
            ...
            return (
                stateName + '<br> Income: ' + '$ '+ inc + ' <br> Obesity: ' + obe +'%'
            );
        });
